...
leaflet(data = palop)%>%
  ...
  addCircleMarkers(lng = ~ long, 
                   ...
                   color = ~pal(confirmed),
                   label = paste(palop$Country.Region,'<br>',"Confirmados:", palop$confirmed,'<br>', "Obitos:",palop$death,'<br>',"Recuperados:",palop$recovered),
...
covid19cvdata::covid19cv %>% mutate(concelho = stringr::str_replace_all(concelho, c("Sal Rei"= "Boa Vista", "Mindelo"="São Vicente"))) %>%
  ...
  DT::datatable(rownames = FALSE,extensions = 'Buttons',
            options = list(dom='Bfrtip',searchHighlight = TRUE, 
                           pageLength = 20,buttons=c('csv'),columnDefs = list(list(className = 'dt-center', targets = 0:4))), filter = 'top')
```
