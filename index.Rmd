...
covid19cv<-covid19cvdata::covid19cv %>% group_by(data,grupo_etario,sexo)%>%dplyr::summarise(confirmados=sum(tipo_caso=="confirmado"),evacuados=sum(tipo_caso=="evacuado"),recuperados=sum(tipo_caso=="recuperado"),obitos=sum(tipo_caso=="obito"))
...
cv_city<-covid19cvdata::covid19cv_cidades

cv_city$cidade<-gsub("Sal Rei", "Boa Vista",cv_city$cidade)

cv_city$cidade<-gsub("Mindelo", "São Vicente",cv_city$cidade)

cv_city$cidade<- as.character(cv_city$cidade)


cabo_verde_map <- rnaturalearth::ne_states(country = "Cape Verde", returnclass = "sf") %>%
  dplyr::select(cidade = name, region, geometry) %>%
...
```{r}
cv_city %>%
dplyr::select(cidade, confirmados,c_ativos_acumulados, recuperados, obitos, confirmados_acumulados) %>%
  dplyr::group_by(cidade) %>%
  ...
  dplyr::mutate(region = factor(cidade , levels = cidade)) %>%
plotly::plot_ly(
          y = ~ region, 
...
<!-- max_date <- max(cv_city$Data) -->
<!-- cv_city %>%  -->
<!--   dplyr::filter(Data == max_date) %>% -->
<!--   dplyr::group_by(cidade,ilha) %>% -->
<!--   dplyr::summarise(casos=sum(confirmados_acumulados)) %>% -->
<!--   dplyr::arrange(-casos) %>% -->
<!--   # dplyr::mutate(country = factor(Country.Region, levels = Country.Region)) %>% -->
<!--   dplyr::ungroup() %>% -->
<!--   plotly::plot_ly(x = ~ cidade, -->
<!--                   y = ~ casos, -->
<!--                   text = ~ casos, -->
<!--                   color = ~ ilha, -->
<!--                   textposition = 'auto', -->
<!--                   type = "bar") %>% -->
<!--   plotly::layout(yaxis = list(title = "Casos Acumulados"), -->
<!--                  xaxis = list(title = ""), -->
<!--                  margin =  list( -->
<!--                    l = 10, -->
<!--                    r = 10, -->
...
```{r}
plotly::plot_ly(data=covid_demo, x=~confirmados_ativos, y=~grupo_etario,color=~sexo) %>% 
  plotly::add_bars(orientation = 'h', hoverinfo = 'text', text = ~confirmados_ativos) %>%
  plotly::layout(bargap = 0.1, barmode = 'normal',
         xaxis = list(tickmode = 'array', tickvals = c(-1000, -500, 0, 500, 1000),
                      ticktext = c('1000', '500', '0', '500', '1000'),title="Nr. Casos Ativos"))
  
```


### Relação entre Casos Confirmados Ativos, Sexo, Grupo Etário
...
```{r map}
cabo_verde_map<-na.omit(cabo_verde_map)
cabo_verde_map %>% 
  mapview::mapview(legend=TRUE,zcol = "c_acumulados") 
```
...
```{r}
covid19cvdata::covid19cv %>% mutate(cidade = stringr::str_replace_all(cidade, c("Sal Rei"= "Boa Vista", "Mindelo"="São Vicente"))) %>%
  dplyr::select(Data = data, Concelho = cidade, Sexo = sexo,`Grupo Etario`= grupo_etario,`Nacionalidade`= nacionalidade, `Tipo de Caso` = tipo_caso)  %>%
...
Este painel da Covid19 fornece uma visão geral do novo surto do novo Coronavírus COVID-19 em Cabo Verde. Este painel foi construído com R usando a estrutura R Markdown.

**Autoria**

O painel foi desenvolvido e é mantido por [Mário Cardoso](www.marovski).

**Dados**

Os dados de entrada para este painel são os pacotes R [covid19cvdata](https://marovski.github.io/covid19cvdata/) e [coronavirus](https://github.com/RamiKrispin/coronavirus) R (versão de desenvolvimento). Os dados e o painel são atualizados diariamente.
...
* Interface do painel - o pacote [flexdashboard](https://rmarkdown.rstudio.com/flexdashboard/).
...
O painel está disponível no Github, livre para qualquer reutilização.

Para qualquer dúvida ou comentário, você pode abrir um [problema](https://github.com/RamiKrispin/italy_dash/issues) ou entrar em contato comigo no [Twitter] (https://twitter.com/Rami_Krispin).